...
#@keystone.skip_unless_has_keystone_credentials()
    message = "no kpatches loaded on EDPM compute node"
...
class KpatchVerificationTest(testtools.TestCase):
    ...
    def test_kpatches_applied_on_compute_nodes(self):
        ...
        if not compute_nodes:
            tobiko.skip_test("No EDPM compute nodes found in podified topology")
        ...
        for node in compute_nodes:
            if not isinstance(node, _topology.EdpmNode):
                LOG.warning("Skipping non-EDPM node: {}".format(node.name))
                continue
            ...
            try:
                ...
                if result.exit_status == 127:
                    raise KpatchCommandNotFoundError(
                        "kpatch command not found on {}".format(node.name))
                ...
                if not output:
                    raise KpatchNotLoadedError(
                        "No kpatches loaded on {}".format(node.name))
                ...
                LOG.exception("Failed to check kpatches on {}".format(node.name))
